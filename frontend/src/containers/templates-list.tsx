...
export default () => {
  ...
  return (
    <>
      <Text mb="8px" size={24} color={colors.text} weight={800}>
        Your templates
      </Text>

      <Loading mb="24px" loading={loading}>
        {templates?.length === 0 ? (
          <Text size={18} color={colors.text} weight={400}>
            You have no templates right now.
          </Text>
        ) : (
          <Table>
            {templates?.map((template) => (
              <Row key={template._id}>
                ...
                <Item w="100%">
                  <Text w="100%" align="left" size={18} weight={400} color={colors.text}>
                    {template.type}
                  </Text>
                </Item>
      ...
      <Flex gap={8} justify="flex-start">
        ...
        <Disabled disabled={name === ""}>
          <Clickable color={colors.yellow} p="8px 16px" onClick={startUpload}>
            <Text size={18} color={colors.white} weight={800}>
              Upload New
            </Text>
          </Clickable>
        </Disabled>
        ...
        <Clickable color="none">
          <Text size={18} color={colors.yellow} weight={800}>
            extend existing one
          </Text>
        </Clickable>
